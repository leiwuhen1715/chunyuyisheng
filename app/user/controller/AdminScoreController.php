...
class AdminScoreController extends AdminBaseController
{
    ...
    public function ajax(){
        ...
        $count = Db::name('user_score_log')->alias('c')->join($join)->where($where)->count();
        $data  = Db::name('user_score_log')->alias('c')->field('c.*,u.user_nickname,u.mobile')->join($join)->where($where)->order(['id'=>'desc'])->paginate($limit);
        $list = $data->items();
