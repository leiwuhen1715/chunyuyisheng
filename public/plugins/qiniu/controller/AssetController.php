...
class AssetController extends PluginBaseController
{
    ...
    public function saveFile()
    {
        ...
        $qiniuconfig = new \Qiniu\Config();
        $bk=new \Qiniu\Storage\BucketManager($auth,$qiniuconfig);
 
        $err=   $bk->rename($config['bucket'],$fileKey,$fileKey.".{$suffix}");
        if($err){
            $this->error($err);
        }
        list($fileinfo,$error)=$bk->stat($config['bucket'],$fileKey.".{$suffix}");
        if($error){
            $this->error($error);
        }
